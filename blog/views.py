...
def create_blog_view(request):
    ...
    if form.is_valid():
        ...
        author = Account.objects.filter(
            email=user.email).first()  # by default this[Account.objects.filter(email=user.email)]
...
def get_blog_queryset(query=None):
    queryset = []
    # a list of all the queries that were entered by the user
    # what we are going to do
    # take whatever they enter and remove all the white space

    queries = query.split(" ")  # ex. python install 2019 = [python, install, 2019]

    for q in queries:
        posts = BlogPost.objects.filter(
            Q(title__contains=q) |
            Q(body__icontains=q)  # icontains will get rid of any capitalization
        ).distinct()  # distinct means that all post that were retrieved are unique
        for post in posts:
